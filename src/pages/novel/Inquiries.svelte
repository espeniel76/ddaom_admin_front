<script>
	...
	$: {
		const test = [...$inquiries.Data.List];
		const test2 = [...$memberDetails.Data.List];
			...
			<tbody class="table-border-bottom-0">
				{#each $inquiries.Data.List as o, index}
					<tr style="text-align:center" id={o.SeqServiceInquiry}>
						<td><input class="form-check-input" type="checkbox" id="defaultCheck3" bind:group={$checkedList} value={o.SeqServiceInquiry} checked={$check} /></td>
