<script>
  ...
  async function fnSearch() {
    await memberInformation.fetchMemberInformation(
      oSearch,
      $paging.pageSize,
      $paging.nowPage
    );
        ...
        <tr style="text-align:center; ">
          ...
          <th width="50" style=""
            >받은 구독 수<button
              ...
              class="btn btn-sm"
              value="SubscribeDESC"
              on:click={fnButton}>▲</button
            >
            <button class="btn btn-sm" value="SubscribeASC" on:click={fnButton}
              >▼</button
            ></th
          >
          <th width="50"
            >소설 등록 수<button
              ...
              on:click={fnButton}>▲</button
            >
            <button class="btn btn-sm" value="CntNovelASC" on:click={fnButton}
              >▼</button
            ></th
          >
          <th width="50"
            >소설 완결 수<button
              type="button"
              class="btn btn-sm"
              value="FinishNovelDESC"
              on:click={fnButton}>▲</button
            >
            <button
              class="btn btn-sm"
              value="FinishNovelASC"
              on:click={fnButton}>▼</button
            ></th
          >
          <th width="50">닉네임</th>
          <th width="50"
            >최종 접속일<button
              ...
              on:click={fnButton}>▲</button
            >
            <button class="btn btn-sm" value="LastDayASC" on:click={fnButton}
              >▼</button
            ></th
          >
          <th width="50"
            >가입일<button
              ...
              on:click={fnButton}>▲</button
            >
            <button class="btn btn-sm" value="CreatedASC" on:click={fnButton}
              >▼</button
            ></th
          >
          <th width="50"
            >탈퇴일<button
              ...
              on:click={fnButton}>▲</button
            >
            <button class="btn btn-sm" value="DeletedASC" on:click={fnButton}
              >▼</button
            ></th
          >
        </tr>
      ...
      <tbody class="table-border-bottom-0">
        {#each quData as o, index}
          ...
          <tr style="text-align:center">
            ...
            <td
              >{o.deleted_yn == 1
                ? Dates.defaultConvert(o.deleted_at)
                : '-'}</td
            >
          </tr>
        {/each}
