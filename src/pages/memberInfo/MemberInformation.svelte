<script>
	...
	import { memberInformation, paging } from "../../stores";
		...
		<table class="table" style="">
			<thead>
				...
				<tr style="text-align:center; ">
					<th width="50"><input class="form-check-input" type="hidden" id="defaultCheck3" /></th>
					<th width="50" style="">No</th>
					<th width="50">상태</th>
					<th width="50">블랙리스트 여부</th>
					<th width="50" style=""
						>받은 구독 수<button type="button" class="btn btn-sm" value="SubscribeDESC" on:click={fnButton}>▲</button>
						<button class="btn btn-sm" value="SubscribeASC" on:click={fnButton}>▼</button></th
					>
					<th width="50"
						>소설 등록 수<button type="button" class="btn btn-sm" value="CntNovelDESC" on:click={fnButton}>▲</button>
						<button class="btn btn-sm" value="CntNovelASC" on:click={fnButton}>▼</button></th
					>
					<th width="50"
						>소설 완결 수<button type="button" class="btn btn-sm" value="FinishNovelDESC" on:click={fnButton}>▲</button>
						<button class="btn btn-sm" value="FinishNovelASC" on:click={fnButton}>▼</button></th
					>
					<th width="50">닉네임</th>
					<th width="50"
						>최종 접속일<button type="button" class="btn btn-sm" value="LastDayDESC" on:click={fnButton}>▲</button>
						<button class="btn btn-sm" value="LastDayASC" on:click={fnButton}>▼</button></th
					>
					<th width="50"
						>가입일<button type="button" class="btn btn-sm" value="CreatedDESC" on:click={fnButton}>▲</button>
						<button class="btn btn-sm" value="CreatedASC" on:click={fnButton}>▼</button></th
					>
					<th width="50"
						>탈퇴일<button type="button" class="btn btn-sm" value="DeletedDESC" on:click={fnButton}>▲</button>
			...
			<tbody class="table-border-bottom-0">
				{#each quData as o, index}
					...
					<tr style="text-align:center">
						...
						<td>{o.cnt_total}</td>
						<td>{o.cnt_finsh}</td>
						<td><a href="/memberInfo/memberInformation/{o.seq_member}/{o.allocated_db}">{o.nick_name}</a></td>
